  ...
  class _ListarPresencasState extends State<ListarPresencas> {
    ...
    Widget _buildMatrizPresencas() {
      return DataTable(
        ...
        rows: membrosFiltrados.map((membro) {
          List<DataCell> cells = [
            DataCell(
              Text(
                membro.nome,
              ...
              FutureBuilder<Map<String, dynamic>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    double porcentagem = snapshot.data?['porcentagem'] ?? 0.0;
      ...
      try {
        ...
        if (snapshot.value != null) {
          ...
          if (values != null) {
            ...
            datasPresencas.sort((a, b) => _parseDate(b).compareTo(_parseDate(a))); // Ordena as datas em ordem decrescente
